...
var sql = "";
var url = "HOSTNAME=yourURL;PORT=port;DATABASE=db;PROTOCOL=TCPIP;UID=username;PWD=password;";
var commands = [];
...
var sydney = new City("Sydney", 4293000, -33.8651, -151.2094, 61);
	...
	function doEverything(err){
		...
		commands.push("\tFirst document -> ", result);
		...
		var condition = "population > 8000000 and longitude > 40.0";
		sql = "select * from " + tableName + " where " + condition;
		var result = conn.querySync(sql);
		
		commands.push("\tFind all documents with: " + condition);
		commands.push("\tFound Documents: ", result);
		...
		commands.push("\tJoined Documents: ", result);
		...
		commands.push("\tDocuments found: ", result);
...
//		vcap parsing for bluemix
//		var vcap_services = JSON.parse(process.env.VCAP_SERVICES);
//		var credentials = vcap_services['altadb-dev'][0].credentials;
//		var ssl = false;
//		var url;
//		if (ssl){ url = credentials.ssl_drda_url; }
//		else { url = credentials.drda_url; }
//		console.log(url);
		}
